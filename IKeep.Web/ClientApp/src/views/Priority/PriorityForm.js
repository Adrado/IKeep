...
const PriorityForm = ({onModify}) => 
{
  const {state, dispatch} = useContext(Functions)

  const Select = () =>
  {
    dispatch({ type: 'SELECT_ROW', data: new Priority()});
  }
  ...
  const [Add, Save, Delete] = usePriorityViewModel(onModify, Select)
  const {values, handleOnChange, onAdd, onSave, onDelete} = useForm(PriorityState, state.selectedRow, Add, Save, Delete);
