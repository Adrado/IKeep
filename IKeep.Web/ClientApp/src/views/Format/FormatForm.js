...
const FormatForm = ({onModify}) => 
{
  const {state, dispatch} = useContext(Functions)

  const Select = () =>
  {
    dispatch({ type: 'SELECT_ROW', data: new Format()});
  }
  ...
  const [Add, Save, Delete] = useFormatViewModel(onModify, Select)
  const {values, handleOnChange, onAdd, onSave, onDelete} = useForm(FormatState, state.selectedRow, Add, Save, Delete);
